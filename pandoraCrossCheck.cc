...
int goodMINERvAMatch=0; int totalMINERvAMatch=0; 
 double offset=0;
  ...
  TH1D *recoHistVertexY= new TH1D("recoHistVertexY","recoHistVertexY",140,-70,70); 
TH1D *recoHistVertexX= new TH1D("recoHistVertexX","recoHistVertexX",140,-70,70); 
TH1D *recoHistVertexZ= new TH1D("recoHistVertexZ","recoHistVertexZ",140,-70,70); 
  ...
  for(long n = 0; n < Nentries; ++n){ 
	...
	caf_chain->GetEntry(n); //Get spill from tree
      ...
      double  Elep=-999; double cosL=-999;
       ...
       if ((abs(end_pos.z)>62 || abs(end_pos.x)>62) && abs(pdg)==211) escapingPions++;
       else if (abs(pdg)==211)  containPiLen->Fill(length);
       if (abs(end_pos.z)<62 && abs(end_pos.x)<62 && abs(pdg)==2212) containPLen->Fill(length);
...
truePionWithRecoIntDirZ->Fill(dirZ); truePionWithRecoIntDirX->Fill(dirX); truePionWithRecoIntDirY->Fill(dirY);
    ...
    if (sr->common.ixn.pandora[nixn].part.pandora[npart].primary==true) showerCorrectness->Fill(correctShower);
