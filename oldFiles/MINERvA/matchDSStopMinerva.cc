...
int matchDSMinerva(std::string input_file_list, std::string output_rootfile){
  ...
  TH1D* goodMatchedPDG=new TH1D("matchGoodPDG","matchGoodPDG",3000,1,3000);
  TH1D* goodMatchedSecPDG=new TH1D("matchGoodSecPDG","matchGoodSecPDG",3000,1,3000);
...
int maxIxnNumber=-999; int maxPartNumber=-999; int maxTypeNumber=-999; int correctTrack=2;
		...
		if (start_z>0 && end_z>0 && ((tpcTrk_start.z)>58 || (tpcTrk_end.z)>58) ){  // Minerva track needs to be downstream and the TPC track exiting, if you want puch through set end_z>300 if not end_z<300
		...
		double extrapY=dirY/dirZ*(extrapdZ)+tpcTrk_end.y-start_y;      // Calculate extrapolated distortions
		double extrapX=dirX/dirZ*(extrapdZ)+tpcTrk_end.x-start_x;
		double diffExtrap=TMath::Sqrt(TMath::Power(extrapY-start_y,2));


		if (dotProductDS<dotProduct && abs(extrapY)<10){ dotProductDS=dotProduct;  // Select the track that is most parallel
		...
		maxIxnMinerva=sr->nd.minerva.ixn[i].tracks[j].truth[0].ixn;
                if (end_z>300 || abs(end_x)>80 || abs(end_y-30)<115 ) punchthrough=1;
                else punchthrough=0;
		}}
		
		}} // Minerva
	 if (dotProductDS>0.9975 && punchthrough==0){    // Sanity check on dot product 
         dotProduct->Fill(dotProductDS);
         deltaX->Fill(deltaExtrapX); deltaY->Fill(deltaExtrapY);	
	if (maxIxnMinerva==maxIxnNumber && maxPartNumber==maxPartMinerva && maxTypeMinerva==maxTypeNumber){ 

if (maxTypeMinerva<3){ goodMatchedPDG->Fill(abs(sr->mc.nu[maxIxnMinerva].prim[maxPartMinerva].pdg));} else goodMatchedSecPDG->Fill(abs(sr->mc.nu[maxIxnMinerva].sec[maxPartMinerva].pdg));



goodMINERvAMatch++; deltaXGood->Fill(deltaExtrapX); deltaYGood->Fill(deltaExtrapY); dotProductGood->Fill(dotProductDS);}
...
goodMatchedPDG->Write();
goodMatchedSecPDG->Write();
