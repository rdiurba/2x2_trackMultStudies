...
std::map<TString, TProfile*> cosLProfiles;
std::map<TString, TProfile*> multProfiles;
...
for (int i = 0; i < branches->GetEntries(); ++i) {
    ...
    // create profile once
    cosLProfiles[name] = new TProfile(
        Form("trueCosL_%s", name.Data()),
        Form("trueCosL_%s", name.Data()),
        6,edges, "S"   // adjust later if needed
    );
        // create profile once
    multProfiles[name] = new TProfile(
        Form("true_multTrkOnly_%s", name.Data()),
        Form("true_multTrkOnly_%s", name.Data()),
        binsMult,edgesMult,"S"   // adjust later if needed
    );
}

    FluxCovarianceReweight rw;
    
rw.GenerateThrows(100);
  ...
  while(caf_list >> tmp){
TFile *rw =
    new TFile(Form("MiniRun65Nusyst100Universes/MiniRun6.5_1E19_RHC.nuweights.%07d.nusyst.root", index));

    TTree* rw_chain = (TTree*)rw->Get("SystWeights");
       Double_t totalWeight[100];
    rw_chain->SetBranchAddress("totWeight",totalWeight);
    // Re-bind addresses for this file
    for (const auto& name : dialNames) {
        rw_chain->SetBranchAddress(name, dialBuffers[name]);
    }
  ...
  for (long n = 0; n < Nentries; ++n) {
    ...
    double mnvOffsetY = 5;
      ...
      for (long unsigned ntrue = 0; ntrue < sr->mc.nu.size(); ntrue++) {
        ...
        rw_chain->GetEntry(gIdx);
        if (cosL > 0.91 && Elep>1) {
            trueCosL->Fill(cosL);
            for (int index=0;  index<100; index++){
            trueCosLVec.at(index).Fill(cosL,totalWeight[index]);
            ...
            for (const auto& name : dialNames) {
            Double_t* w = dialBuffers[name];
            
            for (int u = 0; u < 100; ++u) {
            multProfiles[name]->Fill(truePartTrkOnly, w[u]);
            cosLProfiles[name]->Fill(cosL, w[u]);
    ...
    for (long unsigned nixn = 0; nixn < sr->common.ixn.dlp.size(); nixn++) {
     ...
     // Calculate muon direction based on distance
      ...
      double trueCosL = -999;
        ...
        int trueMatchMult = 0;
        if (biggestMatchIndex>-1) rw_chain->GetEntry(sr->mc.nu[biggestMatchIndex].genieIdx);
       ...
       if (goodInteraction || goodOutOfSigDef) {
          ...
          if (sr->mc.nu[biggestMatchIndex].iscc &&
            ...
            if (goodInteraction) {   
              ...
              for (int index=0;  index<100; index++){ responseCosLVec.at(index).Fill(cosLReco, trueCosL, totalWeight[index]);
            ...
            if (cosLReco > 0.9 && trueCosL > 0.9 &&
              ...
              backtrackTrueMult->Fill(trueMatchMult);
               for (int index=0;  index<100; index++){ responseMultVec.at(index).Fill(trackMult, trueMatchMult, totalWeight[index]);
        ...
        if (cosLReco > 0.9 && (!goodInteraction || trueCosL < 0.9)) {
          track_multBad->Fill(trackMult);
            for (int index=0;  index<100; index++){
            if (biggestMatchIndex>-1)  badRecoMultVec.at(index).Fill(trackMult,totalWeight[index]);
...
for (int i=0; i<7; i++){
  ...
  badRecoCosL->Write();
  trueCosL->Write();
  for (auto& kv : cosLProfiles)
  kv.second->Write();
  for (auto& kv : multProfiles)
  kv.second->Write();
  totalSpills->Write();
