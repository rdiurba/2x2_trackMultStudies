  ...
  for (long n = 0; n < Nentries; ++n) {
    ...
    for (long unsigned nixn = 0; nixn < sr->common.ixn.dlp.size(); nixn++) {
      ...
      for (long unsigned npart = 0;
        ...
        if (std::isnan(sr->common.ixn.dlp[nixn].part.dlp[npart].start.z) || std::isinf(sr->common.ixn.dlp[nixn].part.dlp[npart].start.z))
            continue;
        if (std::isnan(sr->common.ixn.dlp[nixn].part.dlp[npart].end.z) || std::isinf(sr->common.ixn.dlp[nixn].part.dlp[npart].end.z))
            continue;
            ...
            if ((abs(start_pos.z) > 62 || abs(end_pos.z) > 62)) {
              ...
              double dotProductFromCAF=-999;
                ...
                if (sr->nd.trkmatch.extrap[k].larid.ixn!=nixn || sr->nd.trkmatch.extrap[k].larid.reco!=1) continue;
                    ...
                    if (sr->nd.lar.dlp[nixn].tracks[index].start.z!=sr->common.ixn.dlp[nixn].part.dlp[npart].start.z || sr->nd.lar.dlp[nixn].tracks[index].end.z!=sr->common.ixn.dlp[nixn].part.dlp[npart].end.z) continue;
