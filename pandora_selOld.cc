...
TH1D *recoHistVertexZ= new TH1D("recoHistVertexZ","recoHistVertexZ",70,1230,1370); 
  TH1D *diffPip=new TH1D("diffPip","diffPip",20,0,20);
  TH1D *diffPim=new TH1D("diffPim","diffPim",20,0,20);
  TH1D *diffP=new TH1D("diffP","diffP",20,0,20);
  TH1D *diffN=new TH1D("diffN","diff",20,0,20);

      TH1D *diffPipContained=new TH1D("diffPipContained","diffPipContained",20,0,20);
  TH1D *diffPimContained=new TH1D("diffPimContained","diffPimContained",20,0,20);
  TH1D *diffPContained=new TH1D("diffPContained","diffPContained",20,0,20);
  TH1D *diffNContained=new TH1D("diffNContained","diffContained",20,0,20);

      TH1D *diffPipThresh=new TH1D("diffPipThresh","diffPipThresh",20,0,20);
  TH1D *diffPimThresh=new TH1D("diffPimThresh","diffPimThresh",20,0,20);
  TH1D *diffPThresh=new TH1D("diffPThresh","diffPThresh",20,0,20);
  TH1D *diffNThresh=new TH1D("diffNThresh","diffThresh",20,0,20);

    
      TH1D *diffPipContainedThresh=new TH1D("diffPipContainedThresh","diffPipContainedThresh",20,0,20);
  TH1D *diffPimContainedThresh=new TH1D("diffPimContainedThresh","diffPimContainedThresh",20,0,20);
  TH1D *diffPContainedThresh=new TH1D("diffPContainedThresh","diffPContainedThresh",20,0,20);
  TH1D *diffNContainedThresh=new TH1D("diffNContainedThresh","diffContainedThresh",20,0,20);
 ...
 TH1D* recoBacktrackElAr=new TH1D("recoBacktrackElAr","recoBacktrackElAr",20,0,10);
 TH1D* recoBacktrackCoslAr=new TH1D("recoBacktrackCoslAr","recoBackTrackCoslAr",10,0.9,1);
 ...
 TH1D* selProtonXY=new TH1D("selProtonXY","selProtonXY",20,-1,1);
   TH1D* totalSpills=new TH1D("totalSpills","totalSpills",1,0,1);
...
TH1D* trueTrkLen=new TH1D("trueTrkLen","trueTrkLen",20,0,10);
 ...
 TH1D* trueKaonWithRecoIntYZ=new TH1D("trueKaonWithRecoIntYZ","trueKaonWithRecoIntYZ",20,-1,1);
  ...
  TH1D *diffVertexHist=new TH1D("diffVertexHist","diffVertexHist",100,0,10);
  ...
  TH1D *recoTrkLen=new TH1D("recoTrkLen","recoTrkLen",40,0,20);
  ...
  for(long n = 0; n < Nentries; ++n){ 
    ...
    sumPOT=sr->beam.pulsepot/1E13+sumPOT;
       ...
       int truePart=0; int truePartTrkOnly=0; int truePartNoG4=0;
        if (sr->mc.nu[ntrue].targetPDG==1000180400 && sr->mc.nu[ntrue].hitnuc==0 && sr->mc.nu[ntrue].mode==7) std::cout<<sr->mc.nu[ntrue].mode<<std::endl;
       ...
       diffPip->Fill(npipPrimaries);
       diffPim->Fill(npimPrimaries);
       diffP->Fill(npPrimaries);
       diffN->Fill(nneutronsPrimaries);

      npipPrimaries=0;
       npimPrimaries=0;
        npPrimaries=0;
        nneutronsPrimaries=0;
       for (int k=0; k<sr->mc.nu[ntrue].prim.size(); k++){ if (abs(sr->mc.nu[ntrue].prim[k].end_pos.z)>60 || abs(sr->mc.nu[ntrue].prim[k].end_pos.z)<2.5 || abs(sr->mc.nu[ntrue].prim[k].end_pos.x)>60 || abs(sr->mc.nu[ntrue].prim[k].end_pos.x)<2.5 || abs(sr->mc.nu[ntrue].prim[k].end_pos.y)>60  ) continue; if(sr->mc.nu[ntrue].prim[k].pdg==211) npipPrimaries++; if(sr->mc.nu[ntrue].prim[k].pdg==2212) npPrimaries++;  if(sr->mc.nu[ntrue].prim[k].pdg==-211) npimPrimaries++;  if(sr->mc.nu[ntrue].prim[k].pdg==2112) nneutronsPrimaries++;}

       diffPipContained->Fill(npipPrimaries);
       diffPimContained->Fill(npimPrimaries);
       diffPContained->Fill(npPrimaries);
       diffNContained->Fill(nneutronsPrimaries);

                                    npipPrimaries=0;
       npimPrimaries=0;
        npPrimaries=0;
        nneutronsPrimaries=0;
       for (int k=0; k<sr->mc.nu[ntrue].prim.size(); k++){ if (abs(sr->mc.nu[ntrue].prim[k].end_pos.z)>60 || abs(sr->mc.nu[ntrue].prim[k].end_pos.z)<2.5 || abs(sr->mc.nu[ntrue].prim[k].end_pos.x)>60 || abs(sr->mc.nu[ntrue].prim[k].end_pos.x)<2.5 || abs(sr->mc.nu[ntrue].prim[k].end_pos.y)>60  ) continue; 
              auto start_pos=sr->mc.nu[ntrue].prim[k].start_pos;
       auto end_pos=sr->mc.nu[ntrue].prim[k].end_pos;
    ...
    if(sr->mc.nu[ntrue].prim[k].pdg==211) npipPrimaries++; if(sr->mc.nu[ntrue].prim[k].pdg==2212) npPrimaries++;  if(sr->mc.nu[ntrue].prim[k].pdg==-211) npimPrimaries++;  if(sr->mc.nu[ntrue].prim[k].pdg==2112) nneutronsPrimaries++;}

       diffPipContainedThresh->Fill(npipPrimaries);
       diffPimContainedThresh->Fill(npimPrimaries);
       diffPContainedThresh->Fill(npPrimaries);
       diffNContainedThresh->Fill(nneutronsPrimaries);
      ...
      double  Elep=-999; double cosL=-999;
       ...
       if (sr->mc.nu[ntrue].iscc && abs(sr->mc.nu[ntrue].pdg)==14 && abs(pdg)==13 && cosL<0.9 && Elep<1){
     
       Elep=sr->mc.nu[ntrue].prim[primaries].p.E;
       double dir_x=dX/length; double dir_y=dY/length; double dir_z=dZ/length;
           double cosL=dir_x*beam_x+dir_y*beam_y+dir_z*beam_z;
       histEl->Fill(Elep);
       histCosl->Fill(cosL);
	...
	for(long unsigned nixn = 0; nixn < sr->common.ixn.pandora.size(); nixn++){
          ...
          double biggestMatch=-999; int biggestMatchIndex=-999; double maxDotProductDS=-999; double maxDotProductUS=-999;
           ...
           double dirZExiting=-999;   double startZMuonCand=-999; double dirXExiting=-999; double dirYExiting;

          double recoVertexX=sr->common.ixn.pandora[nixn].vtx.x; double recoVertexY=sr->common.ixn.pandora[nixn].vtx.y; double recoVertexZ=sr->common.ixn.pandora[nixn].vtx.z;
     //   if (n==35 || n==83 || n==109 || n==160 || n==192 || n==202 || n==220 || n==233 || n==251 || n==295) std::cout<<recoVertexX<<","<<recoVertexY<<","<<recoVertexZ<<std::endl;  
        if (/*abs(abs(sr->common.ixn.pandora[nixn].vtx.x)-33)<1 || */ abs(sr->common.ixn.pandora[nixn].vtx.x)>59 || abs(sr->common.ixn.pandora[nixn].vtx.x)<5 || abs(sr->common.ixn.pandora[nixn].vtx.y)>57 || abs(sr->common.ixn.pandora[nixn].vtx.z)<5 || abs(sr->common.ixn.pandora[nixn].vtx.z)>59.5)  continue;
           recoVertex2DNoCuts->Fill(sr->common.ixn.pandora[nixn].vtx.x,sr->common.ixn.pandora[nixn].vtx.z);
	  ...
	  if (abs(sr->mc.nu[biggestMatchIndex].pdg)==14 && abs(sr->mc.nu[biggestMatchIndex].iscc)==true  && sr->mc.nu[biggestMatchIndex].targetPDG==1000180400 &&  abs(recoVertexX-trueVtxX)<5 && abs(recoVertexY-trueVtxY)<5 && abs(recoVertexZ-trueVtxZ)<5 && abs(trueVtxX)>5 && abs(trueVtxX)<59 && abs(trueVtxZ)>5 && abs(trueVtxZ)<59.5 && abs(trueVtxY)<57) goodInteraction=true;
         recoHistVertexY->Fill(sr->common.ixn.pandora[nixn].vtx.y); recoHistVertexX->Fill(sr->common.ixn.pandora[nixn].vtx.x); recoHistVertexZ->Fill(sr->common.ixn.pandora[nixn].vtx.z);
	...
	if (length>longestTrk) longestTrk=length;
        if (/*sr->common.ixn.pandora[nixn].part.pandora[npart].primary==true &&*/ length>minTrkLength){   
		partMult++;
                trackMult++; track_length->Fill(length);
		...
		if (start_z>0 && start_z<170 && end_z>280 && ((end_pos.z)>62) ){
		...
		if (dotProductDS<dotProduct && abs(extrapY)<15  && abs(TMath::ATan(dirXMinerva/dirZMinerva)-TMath::ATan(dirX/dirZ))<0.06 && abs(TMath::ATan(dirYMinerva/dirZMinerva)-TMath::ATan(dirY/dirZ))<0.06 && abs(extrapX)<15){ dotProductDS=dotProduct;
		deltaExtrapY=extrapY;
		deltaExtrapX=extrapX;
		dirZExiting=dirZ;
                                                                                                                                                                                                               
        dirXExiting=dirX;                                                                                                                                                                                                                dirYExiting=dirY;
		if (mcOnly){
		...
		if (start_z<0 && end_z>0 && ( (start_pos.z<-62 && end_pos.z>62) || (start_pos.z>62 && end_pos.z<-62))){
        int truthPart=sr->nd.minerva.ixn[i].tracks[j].truth[0].part;
		double dXMnv=(sr->nd.minerva.ixn[i].tracks[j].end.x-sr->nd.minerva.ixn[i].tracks[j].start.x);
		double dYMnv=(sr->nd.minerva.ixn[i].tracks[j].end.y-sr->nd.minerva.ixn[i].tracks[j].start.y);
		double dZMnv=(sr->nd.minerva.ixn[i].tracks[j].end.z-sr->nd.minerva.ixn[i].tracks[j].start.z);
...
//else oneContained=true;
	...
	if ( /*minervaThrough<1  ||*/  maxDotProductDS<0.99) continue;
         		...
         		for(long unsigned npart=0; npart < sr->common.ixn.pandora[nixn].part.pandora.size(); npart++){ //loop over particles
 	//std::cout<<"Containment variable: "<<sr->common.ixn.pandora[nixn].part.pandora[npart].contained<<std::endl;
              //  if (!sr->common.ixn.pandora[nixn].part.pandora[npart].primary) continue; 
               int pdg=sr->common.ixn.pandora[nixn].part.pandora[npart].pdg;
	...
	if (diffVertex>5) continue;
               ...
               double length=TMath::Sqrt(dX*dX+dY*dY+dZ*dZ);
                double dirX=dX/length; double dirY=dY/length; double dirZ=dZ/length;
                recoTrkLen->Fill(length);}

           double cosL=dirXExiting*beam_x+dirYExiting*beam_y+dirZExiting*beam_z;
          recoCosL->Fill(cosL); 
           recoVertex2D->Fill(sr->common.ixn.pandora[nixn].vtx.x,sr->common.ixn.pandora[nixn].vtx.z);

    //std::cout<<n<<","<<sr->meta.lar2x2.event<<","<<sr->meta.lar2x2.subrun<<std::endl;
...
int maxPartNumber=-999; int maxTypeNumber=-999;
   if ( (abs(pdg)!=-2)){
    ...
    int interactionNumber=sr->common.ixn.pandora[nixn].part.pandora[npart].truth[backTrack].ixn;
    double  partTruthOverlap=sr->common.ixn.pandora[nixn].part.pandora[npart].truthOverlap[backTrack];
    if (maxPartTruthOverlap>partTruthOverlap) continue;
    int backtracked=-9999;
    ...
    if (parType<3){
          ...
          auto end_pos=sr->mc.nu[interactionNumber].prim[partNumber].end_pos;
                   ...
                   double dZLen=(end_pos.z-start_pos.z);
                   double lengthPos=TMath::Sqrt(dXLen*dXLen+dYLen*dYLen+dZLen*dZLen);
                if (sr->common.ixn.pandora[nixn].part.pandora[npart].contained==true && lengthPos>minTrkLength) confusionMatrix->Fill(pdgNumber,backtrackedPDG);
   ...
   if (lengthDist>minTrkLength) trackCorrectness->Fill(correctTrack);
    }}
    }
int trueMatchMult=0;
...
//if (goodInteraction)       std::cout<<biggestMatch<<","<<sr->common.ixn.pandora[nixn].vtx.z<<","<<sr->mc.nu[biggestMatchIndex].vtx.z<<","<<start_pos.z<<","<<end_pos.z<<std::endl;
        if (abs(sr->mc.nu[biggestMatchIndex].prim[primaries].pdg)!=13) continue;     
      recoBacktrackElAr->Fill(Elep);
                     double recoCosL=dirXExiting*beam_x+dirYExiting*beam_y+dirZExiting*beam_z;

       recoBacktrackCoslAr->Fill(cosL); responseCosL->Fill(recoCosL,cosL);
       }
	...
	if (cc==1){ 
	   if (mode==1 || mode==1001) track_multQE->Fill(trackMult);
        ...
        bad_origin->Fill(rock);      track_multBad->Fill(trackMult);
         recoVertex2DBad->Fill(sr->common.ixn.pandora[nixn].vtx.x,sr->common.ixn.pandora[nixn].vtx.z);  trueVertex2DBad->Fill(trueVtxX,trueVtxZ);
          recoVertex2DBadYZ->Fill(sr->common.ixn.pandora[nixn].vtx.y,sr->common.ixn.pandora[nixn].vtx.z);
        } 
    }
...
matchTrue_mult->Write();
 ...
 trueKaonEWithRecoInt->Write();
  ...
  diffPipContainedThresh->Write(); diffPimContainedThresh->Write(); diffPContainedThresh->Write(); diffNContainedThresh->Write();

  diffPip->Write(); diffPim->Write(); diffP->Write(); diffN->Write();
  diffPipContained->Write(); diffPimContained->Write(); diffPContained->Write(); diffNContained->Write();
